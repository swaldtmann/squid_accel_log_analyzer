...
# Source: https://gist.github.com/ArthurClune/3250419
...
class SquidLogLine(object):
    ...
    def __init__(self, line, print_human_times=False, print_minimal=False):
        ...
        try:
            list(map(lambda k, v: setattr(self, k, v),
                     SquidLogLine.fields, line.split()))
        except TypeError:
        ...
        self.client = self.remhost # pylint: disable=maybe-no-member
        try:
    ...
    def __str__(self):
        ...
        if self._print_minimal:
            s += "%s %s %s %s %s" % (self.remhost, # pylint: disable=maybe-no-member
                                     self.status[-3:], self.method, self.url, self.type) # pylint: disable=maybe-no-member
        else:
